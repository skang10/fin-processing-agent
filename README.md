...
Real OCR is opt-in and never downloads models during case processing. Install and verify the pinned assets explicitly with `pnpm ocr:setup -- linux-arm64` (or `linux-x64`), then use `compose.ocr.yaml` together with `compose.yaml`. The overlay keeps the default fake Agent and fixture VLM and mounts OCR assets read-only. PDF, JPEG, and PNG full-page paths are accepted on Linux ARM64; `pnpm ocr:smoke` and `pnpm ocr:image-smoke` run bounded component checks. OCR/VLM bounding boxes remain in source-page coordinates for evidence highlighting; optional bounded rendering can minimize or magnify model input without requiring a second recognition pass. Native Linux x64 and end-to-end evidence-overlay acceptance remain deferred.
