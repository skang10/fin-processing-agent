...
Real OCR is opt-in and never downloads models during case processing. Install and verify the pinned assets explicitly with `pnpm ocr:setup -- linux-arm64` (or `linux-x64`), then use `compose.ocr.yaml` together with `compose.yaml`. The overlay keeps the default fake Agent and fixture VLM and mounts OCR assets read-only. PDF, JPEG, and PNG full-page paths are accepted on Linux ARM64; `pnpm ocr:smoke` and `pnpm ocr:image-smoke` run bounded component checks. Precise candidate boxes remain in source-page coordinates through evidence persistence, API projection, and original-document highlighting; missing or full-page locations remain page-level. The current PDF Inspector OCR API does not return word/line boxes, so local-OCR evidence is page-level even though its text is usable. Optional bounded rendering can minimize or magnify model input without requiring a second recognition pass. Native Linux x64 and genuine coordinate-producing runtime acceptance remain deferred.
