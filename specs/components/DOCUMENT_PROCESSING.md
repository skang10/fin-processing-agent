...
`DOC-REQ-061` The initial OCR adapter must translate PDF Inspector's local PP-OCRv6 Small output behind a project-owned interface. The deployment must separately pin and verify the required PDFium, ONNX Runtime, and model assets and must use an offline pre-provisioned model directory for the delivered demo.
