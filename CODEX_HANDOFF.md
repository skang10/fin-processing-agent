...
35. `case-review-tools-3.7.0` / `adaptive-recovery-tools-2.3.0` add real bounded region delivery. Sharp 0.35.4 validates a normalized in-page rectangle, converts it deterministically to source-pixel bounds, and emits a cropped PNG; `render_page_region` and bounded `extract_with_vlm` now receive that crop instead of the committed full page. Optional `run_ocr` regions filter committed coordinate-bearing OCR spans by intersection while retaining source-page normalized coordinates. This change makes no model call. Focused crop/VLM tests (22), the full 173-test `pnpm check`, `pnpm build`, six-case `pnpm dataset:validate`, and all 23 Docker-backed integration tests pass. It does not yet rerun the OCR engine on the crop or persist the crop as a distinct immutable derived artifact; the durable safe result instead retains the immutable full-page source reference, requested region, and processor version.

### What is still fixture-backed

1. Default demo and CI OCR remain deterministic fixture-backed. The explicit `pdf_inspector` mode now uses pinned, verified, pre-provisioned assets and has one Linux ARM64 synthetic end-to-end acceptance result; broader platform and corpus evidence remains pending.
...
**Complete crop-only OCR execution and immutable crop persistence (`BL-003`, `BL-009`).**

Real pixel cropping is implemented for Agent and VLM delivery, and coordinate-bearing committed OCR spans can be filtered to the region. Next move crop production behind an idempotent durable artifact boundary and execute the OCR engine on the crop rather than filtering eager full-page output, then plan corpus-level evidence. The user explicitly deferred native Linux x64 runtime acceptance until the main functional path is complete. Any additional live-model call requires fresh explicit approval immediately before execution.

### Global next steps

After the immediate task, proceed in this order:

1. **Complete and accept crop-only OCR and immutable crop persistence** (`BL-003`, `BL-009`) on Linux ARM64, then measure the frozen corpus. Defer native Linux x64 execution until the main functional path is complete.
...
4. **Finish V1 hardening and demonstration evidence**: durable crop artifacts, native-text coordinates, OS resource and network isolation, observability evidence, browser acceptance coverage, README/demo limitations, and a reproducible acceptance run.

Corpus-level OCR and live-VLM measurement, crop-only OCR execution, immutable crop persistence, and hardened operating-system resource isolation all remain pending. The accepted single-case OCR and live-VLM results do not establish real-world extraction quality or production fitness.
