...
6. `get_extraction_gaps` is replaced by `get_case_manifest` (logical documents, authorized pages, declared requirements, field schemas, structured application data; no document content). The registry is `case-review-tools-3.1.0`, the prompt is `case-review-prompt-3.1.0`, and the budget envelope is `agent-budget-2.0.0` (24 iterations, 40 tool calls, 8 VLM calls, 8 OCR pages, 180 s wall clock). The `USD 0.25` per-case cost cap is unchanged.
7. `components/ADAPTIVE_EXTRACTION_AGENT.md` is updated to version 3.1.0 and `specs/INDEX.md` to 3.2.0 for these changes: the tool catalog, `AGT-REQ-045`, and new `AGT-REQ-168` to `AGT-REQ-177`. The owning specification, not this handoff, is the authority for the tool catalog and requirement semantics.
