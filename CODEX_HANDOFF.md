...
2. `pnpm check` (145 unit and contract tests), `pnpm test:integration` (23 Docker-backed tests), `pnpm dataset:validate`, `pnpm build`, the Review Web production build, and a Docker Compose run of the bundled demo case plus all six golden cases pass. Three explicit `openai/gpt-5.6-terra` acceptance attempts on `golden-001` drove two contract fixes. The confirming run cost USD 0.039304, extracted all seven values correctly, passed all five rules, produced no issues, and committed a ready verified report in 8 iterations and 13 tool calls. A subsequent `golden-006` live run cost USD 0.064112 and produced the expected issue and a verified report, but skipped `render_page_region`; Agent 3.5.0 now requires visual inspection before OCR or VLM on a PDF Inspector `needsOcr` page and needs a confirming multimodal rerun. This accepts the native-text live route for one clear synthetic case, not multimodal, real OCR, real VLM, or corpus-level quality.
