...
59. Native-PDF evidence localization now calls PDF Inspector 1.17.0's explicit `extractTextWithPositions` operation alongside Markdown extraction instead of assuming undocumented positioned items exist on the Markdown result. Positioned text is page-filtered, scaled into the committed render coordinate space, stored in the bounded native-text artifact, and inherited by an exact or containing candidate match as `page_region` evidence. A direct run against frozen `golden-002-employer-conflict` located `Nordwerk Demo GmbH` on page 2 and `Suedwerk Demo KG` on page 3 with non-empty bounding boxes; `pnpm check` passed all 204 tests and the Docker-backed integration suite passed all 25 tests. Existing paid live case `FD-2026-0053` correctly retains its sealed historical page-level evidence and cannot acquire new regions without a new run; no paid rerun was started.
