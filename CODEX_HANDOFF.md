...
57. Review issue presentation now maps every registered deterministic validation code to a semantic reviewer-facing title instead of falling back to `Review issue N`. It distinguishes conflicts from unresolved or incomparable outcomes while remaining compatible with the earlier generic `value_mismatch` reason. In live demo case `FD-2026-0052` (`golden-004-missing-bank-evidence`), the two valid findings now appear as `Bank statement missing` and `Applicant name could not be fully verified`. `pnpm check` passed all 203 tests, the production Web build passed, and a headed Playwright check confirmed both titles in Agent Report and Issues.
58. Causally dependent deterministic findings are now composed into one reviewer task under `UI-REQ-163`: when a required bank statement is missing and that absence makes the primary account-holder name unresolved, the deterministic Report Verifier retains both finding references but emits one missing-document attention item and one persisted review issue. `case-review-prompt-3.6.1` asks the Agent to make the same composition, while verifier-side composition keeps the result deterministic for every model route. Existing untouched synthetic case `FD-2026-0052` was safely backfilled after confirming that neither issue had a human action, edit, or requested-change draft; its immutable original Agent submission and both validation findings remain retained. The live UI now shows `Issues 1` with the explanation that the missing bank statement also prevents account-holder-name verification. `pnpm check` passed all 204 tests, `pnpm build` passed, the seven-case dataset validation passed, and a headed Playwright check confirmed the merged report and issue detail. No case or paid model call was started.
59. Native-PDF evidence localization now calls PDF Inspector 1.17.0's explicit `extractTextWithPositions` operation alongside Markdown extraction instead of assuming undocumented positioned items exist on the Markdown result. PDF Inspector reports positioned-text coordinates from the bottom-left, so the adapter now uses the inspected page height to convert them to the committed render's top-left coordinate space before scaling and persistence. Native-text provenance is versioned as `agent-native-text-reading-1.2.0`; the Review Workbench applies a narrowly scoped display-time conversion only to already sealed `1.1.0` page regions, preserving immutable run evidence while rendering it correctly. Browser verification on `FD-2026-0054` confirmed the page-2 highlight on `Nordwerk Demo GmbH` and the page-3 highlight on `Suedwerk Demo KG`. A direct adapter run produced render-space boxes `[55,270.4167,238.9536,287.2222]` and `[165,470.5556,281.8200,482.7778]`, respectively. `pnpm check` passed all 205 tests and `pnpm build` passed. Existing paid live case `FD-2026-0053` correctly retains its sealed historical page-level evidence and cannot acquire new regions without a new run; no paid rerun was started.
60. The Review queue now paginates its filtered client-side projection at ten cases per page. Search, status-filter, and queue-view changes reset to page one; refresh clamps the active view to a valid page, and accessible previous/next controls expose both the displayed range and total page count. The API response and navigation counts remain whole-view totals.
...
66. Review issues can now be reopened before final review through the additive `POST /api/v1/cases/{case_id}/issues/{issue_id}/reopen` command. The command is idempotent, result-revision-bound, and optimistically concurrent; it appends `reopen_issue`, increments the issue version, preserves prior confirm/ignore actions and requested-change revisions, and returns the current state to pending. Review Workbench presents **Reopen issue** for resolved issues and restores **Ignore issue** / **Confirm issue** after success. Approved owners are Data Model 3.4.0 (`DAT-REQ-192`), API Contracts 1.13.0 (`API-REQ-107`), Review Workbench 3.10.0 (`UI-REQ-177`), and Index 3.15.0. Browser verification reopened the human-created issue in `FD-2026-0056` (`8e663448-471a-45cd-ad3f-c29413ab4a95`): its current state is `pending`, version 4, its application-display-name evidence remains visible, and durable actions retain `accept_signal:3` followed by `reopen_issue:4`. No Agent or paid model ran. `pnpm check` passed 208 tests, the root and Review Web builds passed, the targeted PostgreSQL integration file passed all 6 tests including replay and forbidden-state boundaries, and `git diff --check` passed.
67. Human issue creation now projects the persisted supporting references into the in-memory issue presentation immediately after save, so the new issue shows its evidence without requiring a page refresh. The issue edit, confirm, and ignore forms also remove duplicate back/edit labels and the redundant sticky issue footer while a form is open, leaving one local Cancel/action pair. Browser verification against `FD-2026-0056` confirmed its application-data evidence remains visible and the simplified edit form retains the selected source. No Agent or paid model ran. `pnpm check` passed 208 tests, the Review Web build passed, and `git diff --check` passed.
68. `Generate demo case` no longer navigates into the browser-only preview while durable preparation is pending. The Review queue remains visible with a disabled **Generating case…** action until the API case reaches `ready_for_review`; only then does the workbench navigate to the canonical `case_id`. Browser verification generated `FD-2026-0060` (`75c4d604-ac55-42b3-a94a-f75c2c66f369`): the waiting state remained on the queue, the resulting URL contained the durable identifier, and the Agent Log showed the model selector, **Run agent review**, `Not run`, and cost `—`. No Agent or paid model ran. `pnpm check` passed 208 tests, the Review Web build passed, and `git diff --check` passed.
69. The human issue editor now labels the explicit no-reference path **No supporting evidence**, reveals a required reason with a missing-document example, and presents a persisted no-reference reason as the reviewer evidence note. This uses the existing API and persistence invariant that every issue has either supporting references or an explicit no-reference reason. For resolved issues, **Reopen issue** moved from the sticky primary-action footer into a **Recorded outcome** row beside `Confirmed` or `Ignored`, reflecting that reopening reverses an existing reviewer decision; the footer is then navigation-only. Browser verification on `FD-2026-0060` confirmed the new no-evidence option without saving a test issue. No Agent or paid model ran. `pnpm check` passed 208 tests, the Review Web build passed, and `git diff --check` passed.
70. Selecting **No supporting evidence** now immediately removes the inapplicable **Add** action, reveals and focuses the required **Why no supporting evidence is available** field, and gives a missing-document placeholder; the validation message explicitly directs the reviewer to that path. New human issues now start with empty required fields and grey hints (`Summarize the issue`, `Describe what needs review and why`, and `Describe what information or document is needed`) instead of persistable sample text, while the unsaved list row is labelled `New issue · Human created`. Browser verification on `FD-2026-0060` exercised both states and cancelled without persisting a test issue. No Agent or paid model ran. `pnpm check` passed 208 tests, the Review Web build passed, and `git diff --check` passed.
71. Agent-optional cases now state **Running the Agent will no longer be available after human review begins.** directly below the model and Run controls, making the immutable review-order boundary visible before the reviewer creates an issue. Review & submit now renders `No issues recorded.` as a plain empty state instead of an empty bordered issue list. Browser verification on `FD-2026-0060` confirmed both presentations. No Agent or paid model ran. `pnpm check` passed 208 tests, the Review Web build passed, and `git diff --check` passed.
72. Review & submit removes the redundant **Draft only** label from Message preview and presents the no-message state as plain text without an input-like border, matching the Review summary empty-state treatment. Browser verification on `FD-2026-0060` confirmed the simplified presentation. No Agent or paid model ran. `pnpm check` passed 208 tests, the Review Web build passed, and `git diff --check` passed.
73. The Agent-order warning moved out of the model controls and into the empty Issues state, where it now reads: `No issues recorded. Adding one starts human review and disables Agent review.` This keeps the Agent Log compact while placing the consequence immediately beside **New issue**. Browser verification on `FD-2026-0060` confirmed the message appears only in the applicable empty human-review state. No Agent or paid model ran. `pnpm check` passed 208 tests, the Review Web build passed, and `git diff --check` passed.
74. The pre-session Agent Log no longer appears stalled after document preparation: while the start command is queued but no durable Agent session exists, the timeline shows `Waiting for Agent to start` / `Queued for the Agent worker` as the current step. Pending, running, and persisted Agent Logs now share the same full-width header, timeline padding, scrolling surface, and stop-action alignment, preventing the shell from changing abruptly when full session activity arrives. No Agent or paid model ran during this UI-only verification. `pnpm check` passed 208 tests, the Review Web build passed, and `git diff --check` passed.
75. Running and completed Agent Logs now retain the same compact inset card selected in review: status, fixed run model, and cumulative cost remain in one header, with a single independently scrollable timeline below. Reviewer-facing events use concise task language while retaining the exact tool identifier as secondary metadata; reconciliation is summarized as one accepted-value event and successful report completion as one issue/fact event. The query now binds a processing case to its current run instead of showing the preceding manual/Agent report until the new report seals. Persistence integration coverage confirms the new run is returned as pending immediately after Agent start. Existing completed synthetic case `FD-2026-0065` / `74df8c77-c845-4ca9-a2f2-40098d2af9a1` supplied visual evidence; no new Agent or paid model run was started. `pnpm check` passed 208 tests, `pnpm build`, the Review Web build, `git diff --check`, and all 6 targeted Docker-backed persistence integration tests passed.
76. Issue details no longer reuse the selected-row blue background and accent bar. The selected issue remains visibly blue in the issue list, while its description is presented as neutral body text under a `What needs review` label before the evidence section, making list selection and detail content visually distinct without changing issue data or behavior. The Review Web production build and `git diff --check` passed.
77. The issue-confirmation form removes the redundant `Applicant-facing draft` explanatory callout. The existing `Requested change` field and confirmation action remain unchanged and provide the necessary context without an extra visual block. The Review Web production build and `git diff --check` passed.
78. The Issues workspace now makes master/detail ownership explicit: the selected issue remains highlighted in the navigator, while the detail pane begins with a neutral sticky `Issue XX of YY` context bar containing the issue title and review state. Evidence is rendered as compact source/field/value rows with the navigation action aligned separately instead of large vertically stacked cards, reducing visual weight and preserving the selected-issue context while scrolling. `pnpm check` passed all 208 tests, and the Review Web production build and `git diff --check` passed.
79. The Issues workspace removes the remaining duplicated list-row/detail-header presentation. A single issue now has one compact switcher row containing its title, origin, state, and Edit action; multiple issues use one labelled native selector (`Issue X of Y`) while the existing footer arrows retain sequential navigation. The detail begins directly with `What needs review`, and evidence uses lightweight divided rows rather than bordered cards. `pnpm check` passed all 208 tests, and the Review Web production build and `git diff --check` passed.
80. The issue selector now uses a horizontally scrollable compact card rail instead of a dropdown. Every card exposes its number, short title, origin, and current review state; the selected card has a restrained blue treatment, while a single card fills the available width. Clicking a card changes the neutral detail pane, and Edit moved beside the `What needs review` heading so cards remain navigation rather than mixed navigation/actions. Existing footer arrows continue to support sequential review. `pnpm check` passed all 208 tests, and the Review Web production build and `git diff --check` passed.
81. Issue evidence now uses consistent source rows instead of loosely aligned document metadata. A compact `A` or page-specific `P#` marker identifies application and document evidence; the middle column contains the semantic field or submitted filename with safe truncation, the value/page has a stable right-aligned position, and one short `Open` action performs navigation. This removes redundant `Document evidence` wording and prevents long filenames from breaking alignment. `pnpm check` passed all 208 tests, and the Review Web production build and `git diff --check` passed.
82. The evidence-row source badges (`A` and `P#`) were removed after visual review. Source identity remains explicit in the `Application data` / `Document` text, while field or filename, value or page, and the `Open` action retain their aligned columns. The Review Web production build and `git diff --check` passed.
83. Reviewer evidence projections now include only normalized values from accepted claims linked to current-run evidence. Document evidence rows use `Page N` as the primary location and show the issue-relevant accepted value at the right; `FD-2026-0069` therefore presents page 2 with `2980.00 EUR` for its monthly-income issue instead of repeating the PDF filename and page number. Rule-to-field selection prevents a multi-claim page from displaying an unrelated accepted value. The global sidebar default-model label was removed because it could be mistaken for the immutable model used by the current case; case-level Agent Log metadata remains authoritative. API Contracts 1.14.0 and IDX 3.16.0 own the projection change. `pnpm check` passed all 208 tests, `pnpm build`, the Review Web production build, and `git diff --check` passed, and the targeted Docker-backed persistence integration suite passed all 6 tests. The running synthetic demo returned `income.monthly_net = 2980.00 EUR` for the issue's page-2 evidence. No case or paid model call was started.
84. The hard-coded `LK` control was replaced with a non-interactive generic reviewer avatar labeled `Demo reviewer`; it no longer implies a real authenticated identity or unavailable profile menu. Active Agent logs add a 24 px separation between the scrollable timeline and the Stop action so the destructive control reads as a distinct action rather than another log entry. `pnpm check` passed all 208 tests, and the Review Web production build and `git diff --check` passed.
85. Expanded Checked Fact sources no longer render an empty-value suffix such as `Page 1: —`. A document reference with no accepted extracted value shows only its page number; the value separator appears only when a safe value is present. The redundant top-bar synthetic-demo badge and non-functional Help control were removed; the visible synthetic markings on every source document and the enforced synthetic-only data boundary remain unchanged. `pnpm check` passed all 208 tests, and the Review Web production build and `git diff --check` passed.
86. Issue navigation again focuses the selected issue's primary page-region evidence when an accepted bbox exists. Entering Issues from the tab or Agent Report, selecting an issue card, and using previous/next navigation all restore the matching overlay; page-level evidence continues to open the correct page without inventing a highlight. `pnpm check` passed all 208 tests, and the Review Web production build and `git diff --check` passed. A real-browser check on `FD-2026-0054` confirmed the automatic employer overlay on page 2 and the direct evidence overlay after opening page 3.
87. `Waiting for Agent to start` is now a stable Agent-log milestone rather than a provisional row that disappears when the durable session is created. It remains the sole current step until `Started Agent review` appears, then stays in the completed timeline immediately before the durable start event. Single and multiple issue cards now share the same 220 px width. Missing-document evidence uses a compact, regular-weight `No page evidence` note instead of presenting the explanation as oversized bold evidence text. `pnpm check` passed all 208 tests, and the Review Web production build and `git diff --check` passed.
88. A newly created issue remains an explicit `Unsaved` / `Draft` item until its create command succeeds. Switching to another issue card or using previous/next is blocked while the draft form is open, with a concise instruction to save or cancel; Cancel removes the local draft as before. The same navigation guard prevents losing an in-progress edit, confirmation, or ignore form. A real-browser check on `FD-2026-0054` confirmed that the draft remains selected and editable and that the attempted switch shows the guard message without issuing a create command.
