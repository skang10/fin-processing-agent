...
28. Live-model evaluation capture now requires `cost_budget.maximum_total_usd` and `cost_budget.maximum_per_case_usd`. The coordinator reserves at least the delivered USD 0.25 case ceiling before starting each case, reconciles the persisted Agent Log USD cost after it, rejects a breached per-case or total limit, and records all remaining cases as `whole_run_cost_budget_exhausted` or `whole_run_cost_unavailable` instead of starting them. The capture output is atomically reserved as `capture_in_progress` before any case submission so the same output target cannot silently repeat paid work after interruption. Offline fake-model capture remains compatible without a cost budget. Evaluation reports retain the configured cost budget and structured excluded-case reasons, and live sessions take their cumulative USD cost from durable session usage rather than the legacy report-level EUR projection. `pnpm check` passes 156 tests and `pnpm test:integration` passes 23 tests.
...
**Run one explicitly approved, budgeted live evaluation case and capture its operational evidence.**

The whole-run coordinator cap is implemented, but no new paid run has been authorized or executed. Ask for explicit approval immediately before the next live call. Start with one synthetic attention case under both the USD 0.25 per-case cap and an equal USD 0.25 whole-run cap; do not expand to multiple cases until that fail-closed path is evidenced.

### Global next steps

After the immediate task, proceed in this order:

1. **Evidence the whole-run live-evaluation budget on one explicitly approved synthetic attention case**, capturing latency, model calls, token usage, estimated cost, and unavailable values required by `MLE-REQ-070` and `MLE-REQ-071`.
