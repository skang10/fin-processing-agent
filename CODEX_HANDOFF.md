...
**Run the two-configuration VLM selection benchmark (`BL-004`) only after explicit approval for its paid calls.**

The real offline OCR runtime is now accepted for one Linux ARM64 synthetic case, but corpus-level OCR quality and other platforms remain unproven. Any VLM benchmark call is paid and still requires fresh explicit approval immediately before execution.

### Global next steps

After the immediate task, proceed in this order:

1. **Run the two-configuration VLM selection benchmark** (`BL-004`) and complete ADR-003 from measured evidence rather than adopting `gpt-5.6-terra` by preference; obtain explicit paid-run approval first.
2. **Broaden real OCR acceptance** (`BL-003`) across the remaining supported platform, JPEG/PNG and crop/region paths, then measure the frozen corpus.
