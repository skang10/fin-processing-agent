...
**Adopt and accept a genuine coordinate-producing extraction path, then measure the frozen corpus (`BL-009`, `BL-006`).**

The bbox persistence/API/UI chain is implemented without repeated recognition, and its overlay geometry is browser-accepted with a non-persistent fixture. The current PDF Inspector OCR boundary cannot supply word/line coordinates, so next either adopt an approved coordinate-bearing local OCR output or perform a fresh explicitly approved live-VLM bbox run. Retain page-level presentation when no precise bbox exists, and do not mutate or backfill retained cases. The user explicitly deferred native Linux x64 runtime acceptance until the main functional path is complete. Any additional live-model call requires fresh explicit approval immediately before execution.

### Global next steps

After the immediate task, proceed in this order:

1. **Accept a genuine coordinate-producing extraction path** (`BL-009`) through the implemented overlay, then measure the frozen corpus. Defer native Linux x64 execution until the main functional path is complete.
...
Corpus-level OCR and live-VLM measurement, genuine coordinate-producing runtime acceptance, and hardened operating-system resource isolation remain pending. The overlay geometry itself is browser-accepted; the current local OCR API supplies no precise coordinates. Crop-only re-recognition is deliberately not a default objective without measured need. The accepted single-case OCR and live-VLM results do not establish real-world extraction quality or production fitness.
