...
Record the known `golden-003` and `golden-004` divergences without editing frozen truth, then decide through the dataset review workflow whether to regenerate `golden-003` or confirm a new release. Keep live routes opt-in; do not extend paid evaluation until a whole-run cost cap exists.
