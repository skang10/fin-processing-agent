...
84. The hard-coded `LK` control was replaced with a non-interactive generic reviewer avatar labeled `Demo reviewer`; it no longer implies a real authenticated identity or unavailable profile menu. Active Agent logs add a 12 px separation between the scrollable timeline and the Stop action so the destructive control reads as a distinct action rather than another log entry. `pnpm check` passed all 208 tests, and the Review Web production build and `git diff --check` passed.
