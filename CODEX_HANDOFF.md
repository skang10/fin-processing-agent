...
22. Review of the confirming result found that `case-normalization-1.0.0` rejected the correctly observed raw value `EUR 2980.00` because it supported the `€` symbol but not the ISO `EUR` prefix. The VLM and candidate evidence boundary were correct, but the payslip income did not become a claim, so the immutable run above reported `inconclusive` / `income_input_incomparable` instead of the semantically correct `failed` / `income_conflict` for application EUR 3050.00 versus payslip EUR 2980.00. `case-normalization-1.1.0` now accepts explicit EUR prefixes and suffixes, rejects unsupported or duplicate currency markers, and has regression coverage through deterministic assembly and the income rule. The prior run remains evidence for live extraction, trace safety, budget persistence, and restart reuse, but it is not final acceptance evidence for income-result semantics. A confirming end-to-end run remains necessary; do not start another paid run until explicitly approved and protected by the existing per-case cap.
...
**Confirm `case-normalization-1.1.0` end to end, then capture a fresh offline Agent-led evaluation baseline against frozen release `v0.1.1`.**

First prove that a raw `EUR 2980.00` candidate produces an income claim and `failed` / `income_conflict`. Any new paid live run still requires explicit approval and the USD 0.25 per-case cap. Then record the known `golden-003` and `golden-004` divergences without editing frozen truth and decide through the dataset review workflow whether to regenerate `golden-003` or confirm a new release.
