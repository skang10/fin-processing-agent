...
4. Native-text candidates carry a page reference without a bounding box because the native-text boundary does not expose layout coordinates. The opt-in patched local-OCR binding and precise model-extraction candidates carry the region their source returned.
...
**Measure the frozen corpus with the accepted coordinate-producing extraction path (`BL-009`, `BL-006`).**

The bbox persistence/API/UI chain is now accepted end to end with genuine same-pass local-OCR polygons on one frozen synthetic case. Next measure coordinate coverage and quality across the frozen corpus while retaining page-level presentation when no precise bbox exists; do not mutate or backfill retained cases. The user explicitly deferred native Linux x64 runtime acceptance until the main functional path is complete. Any additional live-model call requires fresh explicit approval immediately before execution.

### Global next steps

After the immediate task, proceed in this order:

1. **Measure the accepted coordinate-producing path** (`BL-009`) across the frozen corpus. Defer native Linux x64 execution until the main functional path is complete.
...
4. **Finish V1 hardening and demonstration evidence**: native-text coordinates, OS resource and network isolation, observability evidence, browser acceptance coverage, README/demo limitations, and a reproducible acceptance run.

Corpus-level OCR and live-VLM measurement and hardened operating-system resource isolation remain pending. Genuine same-pass local-OCR coordinate delivery and click-activated overlay behavior are accepted for one frozen synthetic case; native-text coordinates remain unavailable. Crop-only re-recognition is deliberately not a default objective without measured need. The accepted single-case OCR and live-VLM results do not establish real-world extraction quality or production fitness.
