...
1. The repository is on `main`. Commit `8d538a6` (`feat: wire live page VLM extraction`) adds the opt-in live page-VLM route, schema boundary, configuration, durable usage accounting, and documentation. The worktree was clean before this handoff update.
2. `pnpm check` (148 unit and contract tests), `pnpm test:integration` (23 Docker-backed tests), `pnpm dataset:validate`, `pnpm build`, the Review Web production build, and a Docker Compose run of the bundled demo case plus all six golden cases pass. Three explicit `openai/gpt-5.6-terra` acceptance attempts on `golden-001` drove two contract fixes; the confirming run cost USD 0.039304 and committed the expected verified report. A confirming `golden-006` run under Agent 3.5.0 cost USD 0.084095, delivered all three `needsOcr` page images to the live model before OCR/VLM, retained no image markers in durable safe output, produced the expected income issue, and committed a verified report in 11 iterations and 22 tool calls. The new isolated live VLM adapter separately recognized `EUR 2980.00` from the synthetic scanned payslip page with `openai/gpt-5.6-terra` at 1,507 input tokens, 14 output tokens, and USD 0.003934. This accepts the adapter call and schema boundary, not the still-pending end-to-end live VLM route, real OCR, corpus-level quality, or production fitness.
...
19. One paid end-to-end attempt was run in an isolated PostgreSQL database and MinIO bucket with Compose providing infrastructure and pnpm providing API, Worker, and Review Web. Case `357dfaed-5244-4869-b3e0-9292b423b88a`, run `b8406fcc-3065-4228-82ee-a09187dd2880`, session `9bdfa126-fb91-47b7-82a4-d7e39433416d` used `openai/gpt-5.6-terra`, `case-review-prompt-3.4.0`, `case-review-tools-3.5.0`, `extract_with_vlm` 3.1.0, and `page-field-extraction-1.1.0`. It visually inspected all three `needsOcr` pages, ran the approved OCR boundary on all three, and made eight live VLM calls; no `fixture-vlm-gateway` marker or encoded page/document bytes occurred in durable tool output. The first seven calls correctly returned the identity holder, expiry, employee, payslip employer, `EUR 2980.00`, account holder, and then incorrectly returned the account-holding bank for the generic bank-statement `organization.name` request. Candidate submission was atomically rejected because the proposed salary counterparty did not appear verbatim in that VLM output; a repeated generic organization request returned the bank again, and the session terminated `tool_budget_exhausted`. The run ended `processing_exception` with no sealed result, finding, disposition, or report. Persisted cumulative usage was 7 iterations, 18 charged tool calls, 15 model calls, 8 VLM calls, 3 OCR pages, 12,096 input tokens, 2,189 output tokens, and USD 0.095664. The nested VLM portion was 12,075 input tokens, 103 output tokens, and USD 0.0314115. Because the first run did not succeed, Worker restart/re-entry was not exercised. The trusted requirement-specific VLM contract fix described in item 6 is implemented and passes offline checks, but a second paid run was deliberately not started because the project still lacks a whole-run live-evaluation cap. End-to-end live-VLM acceptance therefore remains pending.
...
**Confirm the requirement-specific VLM contract fix with one approved `golden-006-scanned-adaptive-unavailable` paid run, then exercise compatible Worker re-entry.**

1. Read `AGENTS.md`, this handoff, `LIMITATIONS.md`, the specification index, and the owning Agent, document-processing, security, evaluation, deployment, and VLM ADR documents before changing behavior.
2. Reuse or re-establish the coherent topology proven by the failed attempt: PostgreSQL and MinIO as infrastructure services with API, Worker, and Review Web launched through pnpm. Do not silently change the delivered deployment baseline.
3. Start the newly built Worker with the ignored `.env` loaded, and confirm its startup log reports `vlm_mode=live` and `vlm_model=openai/gpt-5.6-terra` without printing credentials.
4. Use a new isolated database and bucket and load only `golden-006-scanned-adaptive-unavailable`. Do not mutate or reuse the terminal failed run, delete broad paths or volumes, or modify frozen truth.
5. Verify the Agent visually inspects each `needsOcr` page, attempts the approved local path, and then invokes `extract_with_vlm` through `openai/gpt-5.6-terra`; no `fixture-vlm-gateway` value may enter this run.
6. Verify all accepted candidates appear verbatim in the corresponding VLM tool output, deterministic reconciliation and the five-rule registry run afterward, and the final result contains the expected `VAL_INCOME_CONSISTENCY_001` outcome without a lending decision.
7. Query durable session/tool state and the reviewer Agent Log to prove page images and document bytes were not persisted, the VLM prompt/model versions were retained, and nested VLM tokens and USD cost were added to the cumulative session budget.
8. Exercise one compatible Worker restart/re-entry if the initial run succeeds, confirming that the committed paid VLM result is reused and its usage is neither repeated nor reset.
9. This is another paid run. Retain the USD 0.25 per-case cap, obtain explicit user approval before starting it, and do not run multiple live cases until a separate whole-run cap exists. Explicitly disclose the prior USD 0.095664 attempt when requesting approval.
10. After acceptance, update this handoff with case/run identifiers, exact result, model/prompt/tool versions, calls, tokens, cost, restart evidence, and remaining limitations; run `pnpm check`, `pnpm build`, `pnpm dataset:validate`, and relevant integration checks before committing.
...
Real OCR recognition, live VLM acceptance and corpus-level measurement, crop rendering, and hardened operating-system and network isolation all remain pending.
