...
**Implement real crop/region rendering and OCR (`BL-003`, `BL-009`).**

PDF, JPEG, and PNG full-page OCR paths are accepted on Linux ARM64. The user explicitly deferred native Linux x64 runtime acceptance until the main functional path is complete. Next replace the current full-page fallback for bounded region requests with real crop rendering and region OCR, then plan corpus-level evidence. Any additional live-model call requires fresh explicit approval immediately before execution.

### Global next steps

After the immediate task, proceed in this order:

1. **Implement and accept crop/region rendering and OCR** (`BL-003`, `BL-009`) on Linux ARM64, then measure the frozen corpus. Defer native Linux x64 execution until the main functional path is complete.
...
4. **Finish V1 hardening and demonstration evidence**: crop rendering, native-text coordinates, OS resource and network isolation, observability evidence, browser acceptance coverage, README/demo limitations, and a reproducible acceptance run.
