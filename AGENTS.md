...
The bounded Pi harness can visually inspect an authorized uploaded-document page or optional real pixel crop through `render_page_region`: image bytes are transient multimodal tool content, while durable Agent state retains only safe metadata. OCR and VLM evidence regions are source-page normalized bounding boxes used for candidate provenance and reviewer highlighting; they do not trigger a second recognition pass. The Worker still generates full-page renders and selective OCR eagerly before the session, so demand-driven sandbox execution remains pending and must not be claimed as implemented.
