...
27. `pnpm ocr:setup -- PLATFORM` — download and SHA-256-verify pinned PDFium, ONNX Runtime, and PP-OCRv6 Small assets into ignored local directories for `linux-x64` or `linux-arm64`.
28. `pnpm ocr:verify -- PLATFORM` — verify installed OCR runtime assets against the committed manifest without network access.
29. `pnpm ocr:smoke` — run the real offline OCR component smoke check in a compatible environment with the three OCR runtime paths configured.
30. `pnpm ocr:image-smoke` — recognize bounded synthetic JPEG and PNG inputs through the pinned offline OCR runtime and print safe aggregate evidence.
31. `pnpm ocr:image-acceptance [CASE_ID]` — submit one derived synthetic mixed-image case to a running real-OCR demo, or re-verify an existing runtime case without resubmission.
