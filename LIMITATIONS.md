...
3. The Pi-based Case Review Agent runs as a bounded pre-screening step for every processable case. Optional adaptive extraction is limited to eligible gaps. It cannot access a shell, arbitrary files, unrestricted networks, core banking systems, or policy mutation tools. The default demo and CI paths drive the embedded Pi harnesses with a deterministic scripted fake model rather than a real language model, so Agent report and recovery quality have not been measured. The adaptive-recovery tools in the demo call fixture OCR and VLM adapters that return synthetic values rather than recognizing anything, the live model route has not completed acceptance, and Agent steps are persisted at session end rather than incrementally.
