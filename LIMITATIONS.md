...
9. Bounded page-region rendering now crops the committed PNG and sends only those transient pixels to the Agent or Vision Language Model. Region OCR currently filters committed coordinate-bearing OCR spans to the requested region; it does not rerun the OCR engine on the crop. Crops are reproducible from the immutable full-page render and retained region metadata, but are not yet stored as separate immutable derived artifacts.
