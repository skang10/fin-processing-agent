...
9. Bounded page-region rendering can crop the committed PNG when input minimization or magnification is actually needed. The ordinary small-document path recognizes a page once and uses source-page normalized OCR or VLM bounding boxes as evidence for reviewer highlighting; it does not crop and recognize the same content a second time. The optional crop path has not completed immutable derived-artifact acceptance.
