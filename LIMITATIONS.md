...
7. The default synthetic demo OCR mode still emits deterministic fixture text for pipeline testing and must not be treated as recognition evidence. An explicit opt-in route installs and verifies pinned PDFium 153.0.7988.0, ONNX Runtime 1.27.0, and PP-OCRv6 Small `oar-ocr-v0.7.0` assets and runs PDF Inspector 1.17.0 offline. The frozen six-case synthetic corpus and a derived JPEG/PNG case have completed Linux ARM64 execution; this is runtime and bounded synthetic-recognition acceptance, not representative OCR quality or real-document fitness evidence. Native Linux x64 execution remains unaccepted.
8. The default demo and continuous-integration route remains an explicit fixture adapter (`fixture-vlm-gateway`, `fixture-scanned-page-adapter`) for registered synthetic pages with no committed native text; it recognizes nothing, and no accuracy claim may be derived from it. An opt-in provider-neutral live gateway can instead send one selected committed page render to a configured model with no tools and validate bounded JSON output. ADR-003 selects `openai/gpt-5.6-terra` as the current live default and `openai/gpt-5.6-sol` as fallback after both passed one frozen synthetic scanned case; this same-provider, one-case result is not corpus-level accuracy, provider-resilience, real-world document, or production-fitness evidence. The fixture still supplies page type for registered image-only synthetic pages; an unregistered image-only page may remain unclassified and unresolved.
9. Bounded page-region rendering can crop the committed PNG when input minimization or magnification is actually needed. A precise non-full-page candidate box is persisted as page-region evidence for original-document highlighting; missing and full-page regions remain visibly page-level. A source-pinned compatibility patch exposes native text items retained by the initial PDF parse and OCR polygons retained by the existing OAR pass, so accepted native/OCR candidates can carry same-pass regions without a second parse or recognition call. Results sealed before page-region persistence was implemented are immutable and do not retroactively acquire bounding boxes. Coordinate evidence is accepted on the bounded synthetic Linux ARM64 corpus, not representative real documents. The optional crop path has not completed immutable derived-artifact acceptance.
...
3. The Pi-based Case Review Agent runs as one bounded pre-screening session for every processable case and leads document extraction: the system inspects, renders, and selectively OCRs the pages, but it has no deterministic field parser, so every document value reaches the deterministic pipeline as an Agent candidate produced through a registered, scoped tool. The Agent cannot access a shell, arbitrary files, unrestricted networks, core banking systems, or policy mutation tools. The default demo and CI paths drive the embedded Pi harness with a deterministic scripted fake model and use a fixture VLM adapter, so corpus-level Agent and VLM recognition quality has not been measured. The explicit live VLM route has completed one synthetic scanned end-to-end acceptance case, but this does not establish general recognition quality. Agent steps, tool results, and consumed budgets are persisted as the session runs, so a lost Worker resumes from committed state; a tool call that completed but whose step had not yet committed may repeat, which is why the deterministic components it calls are idempotent for the run.
4. Page classification of the synthetic corpus uses visible demo headings or bounded OCR content signals. In default fixture OCR mode, a registered image-only synthetic page may still use the explicit fixture page-type declaration; an unregistered unreadable page remains unclassified and uncertain. The accepted real-OCR case classified all three scanned pages from OCR content without the fixture declaration.
5. Agent suggestions are limited to registered document-review actions, must cite persisted evidence or findings, and require human confirmation; they are not credit, lending, AML, or KYC advice.
6. Cases with unresolved required evidence cannot be marked ready for downstream processing.
7. Human corrections are retained as dataset candidates but do not automatically update prompts, models, rules, or thresholds.
