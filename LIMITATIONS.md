...
7. The default synthetic demo OCR mode still emits deterministic fixture text for pipeline testing and must not be treated as recognition evidence. An explicit opt-in route now installs and verifies pinned PDFium 153.0.7988.0, ONNX Runtime 1.27.0, and PP-OCRv6 Small `oar-ocr-v0.7.0` assets and runs PDF Inspector 1.17.0 offline. One Linux ARM64 execution over one three-page synthetic scanned case completed end to end; this is runtime and bounded synthetic-recognition acceptance, not corpus-level OCR quality or real-document fitness evidence.
