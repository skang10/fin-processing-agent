...
8. No Vision Language Model gateway is wired yet. For a registered synthetic case, a page that carries no committed native text is answered by an explicit fixture adapter (`fixture-vlm-gateway`, `fixture-scanned-page-adapter`) that returns the values the generator drew on that page image, and the same adapter declares that page's document type. This demonstrates the bounded page-scoped extraction path only; it recognizes nothing, and no accuracy claim may be derived from it. The Agent Log names the gateway on every such step. A non-fixture case receives no such answer: its image-only pages simply stay unresolved.
...
3. The Pi-based Case Review Agent runs as one bounded pre-screening session for every processable case and leads document extraction: the system inspects, renders, and selectively OCRs the pages, but it has no deterministic field parser, so every document value reaches the deterministic pipeline as an Agent candidate produced through a registered, scoped tool. The Agent cannot access a shell, arbitrary files, unrestricted networks, core banking systems, or policy mutation tools. The default demo and CI paths drive the embedded Pi harness with a deterministic scripted fake model and use a fixture VLM adapter, so Agent and VLM recognition quality has not been measured. An explicit live VLM route can send one selected committed page render to a configured provider/model with no tools and locally validate its bounded JSON output, but that route has not completed acceptance. One clear native-text case and one scanned synthetic case previously completed the live Agent route with verified reports; the scanned case proved bounded page-image delivery to the Agent, not VLM recognition quality. Agent steps, tool results, and consumed budgets are persisted as the session runs, so a lost Worker resumes from committed state; a tool call that completed but whose step had not yet committed may repeat, which is why the deterministic components it calls are idempotent for the run.
