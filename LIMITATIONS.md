...
8. The default demo and continuous-integration route remains an explicit fixture adapter (`fixture-vlm-gateway`, `fixture-scanned-page-adapter`) for registered synthetic pages with no committed native text; it recognizes nothing, and no accuracy claim may be derived from it. An opt-in provider-neutral live gateway can instead send one selected committed page render to a configured model with no tools and validate bounded JSON output. One synthetic scanned case has completed this end-to-end live route with correct deterministic income-conflict semantics, durable usage accounting, and restart/re-entry without another model call. That single case is not corpus-level accuracy, real-world document, or production-fitness evidence. The fixture still supplies page type for registered image-only synthetic pages; an unregistered image-only page may remain unclassified and unresolved.
