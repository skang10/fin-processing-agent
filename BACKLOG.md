...
2. Page-region persistence, API projection, and original-document overlay are implemented for precise OCR/VLM candidate regions. Attach normalized evidence regions to native-text candidates; PDF Inspector layout coordinates are not exposed through the current native-text boundary, so those candidates correctly remain page-level.
