...
2. Page-region persistence, API projection, and original-document overlay are implemented for precise candidate regions. The opt-in real-OCR runtime builds a source-pinned compatibility patch for PDF Inspector 1.17.0 that exposes polygons already produced by the same OAR pass. A frozen six-case Linux ARM64 diagnostic produced precise regions for all 10 accepted OCR-derived document claims, with no invalid geometry or missing coordinate lineage; 28 native-text claims remained page-level, so overall accepted document-claim region coverage was 10/38 (26.3%). This synthetic diagnostic is not a comprehensive OCR benchmark. The next coordinate gap is the approved native-text boundary, which still exposes no layout coordinates.
