...
2. Page-region persistence, API projection, and original-document overlay are implemented for precise candidate regions. PDF Inspector 1.17.0 does not expose word/line boxes from `processPdfWithOcr`, and the current native-text boundary likewise exposes no layout coordinates, so both paths correctly remain page-level. Adopt a coordinate-bearing approved OCR output before claiming local-OCR highlights; VLM regions can use the implemented path when the model actually returns a precise non-full-page box.
