...
2. Page-region persistence, API projection, and original-document overlay are implemented for precise candidate regions. The opt-in source-pinned compatibility patch for PDF Inspector 1.17.0 exposes both native items already held by the initial PDF parse and polygons already produced by the same OAR pass. A fresh frozen six-case Linux ARM64 diagnostic produced precise regions for all 30 accepted native-text claims and all 10 accepted OCR-derived claims, with no second parse or recognition pass. This synthetic diagnostic is not a comprehensive coordinate-quality or OCR benchmark; broader document variation remains necessary.
