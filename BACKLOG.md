...
2. Page-region persistence, API projection, and original-document overlay are implemented for precise candidate regions. The opt-in real-OCR runtime now builds a source-pinned compatibility patch for PDF Inspector 1.17.0 that exposes polygons already produced by the same OAR pass; one synthetic PDF case has completed click-activated local-OCR highlighting without repeated recognition. The current native-text boundary still exposes no layout coordinates and correctly remains page-level. Broader coordinate quality measurement remains pending.
